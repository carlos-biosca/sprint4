...
const getMoviesFromDirector = (movies, searcheddirector) => [...movies.filter((movie) => movie.director === searcheddirector)];
...
const orderByYear = (movies) => [...movies.sort((a, b) => a.year > b.year ? 1 : -1)];
// Exercise 6: Calculate the average of the movies in a category
const moviesAverageByCategory = (movies, searchedGenre) => {
    let scores = [...movies.filter((movie) => movie.genre.includes(searchedGenre) && typeof movie.score !== 'string')].map((movie) => movie.score);
    return scores.reduce((a, b) => a + b) / scores.length;
};
...
const bestFilmOfYear = (movies, searchedYear) => movies.filter((movie) => movie.year === searchedYear).sort((a, b) => a.score < b.score ? 1 : -1).slice(0, 1);
